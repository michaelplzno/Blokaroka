...
class Blok
{
  public:
    Blok(COLORREF color, int x, int y)
        : m_wColor(color), m_iX(x), m_iY(y), m_iRenderX(x), m_iRenderY(y), m_iMark(0), m_poLeft(NULL), m_poRight(NULL),
          m_poTop(NULL), m_poBottom(NULL), m_fBottomWeightHeld(-1.0f), m_fTopWeightHeld(-1.0f), m_iDistance(-1),
          m_iRenderState(BRS_SOLID), m_fBlockWeight(0), m_poTopDesiredMate(NULL), m_poBottomDesiredMate(NULL),
          m_iGroup(0), m_physicsBodyId() {};
    ...
    bool CanMateWithBlok(
        Blok *poOther); // Tests the size of the gap between this Blok and poOther to see if a connection can be made.
    Blok *FindMate();   // Recursively iterates over this structure to look for any potential prick that this Blok can
                        // connect to.
    void Mate();        // Connect this Blok with the stored m_poTopDesiredMate or m_poBottomDesiredMate

    bool CanDetachUp(); // Recursivly iterates over this structure to see if detaching the Bloks above will free a new
                        // sub structure.
    void DetachUp();    // Detach all the top Bloks connected to this Blok.

    bool CanDetachDown(); // Recursivly iterate over this structure to see if detaching the Bloks below will free a new
                          // sub structure.
    ...
    void SetBlokRenderPosition(int x, int y); // Set the display position of the Blok, this is where we see the Blok and
                                              // is adjusted when Bloks are lined up to connect.

    void AttachLeft(
        Blok *poLeft,
        bool bMoveOther = true); // Add a new left Blok, if bMoveOther the other Blok is positioned correctly.
    void AttachRight(
        Blok *poRight,
        bool bMoveOther = true); // Add a new right Blok, if bMoveOther the other Blok is positioned correctly.
    void AttachTop(Blok *poTop,
                   bool bMoveOther = true); // Add a new top Blok, if bMoveOther the other Blok is positioned correctly.
    void AttachBottom(
        Blok *poBottom,
        bool bMoveOther = true); // Add a new bottom Blok, if bMoveOther the other Blok is positioned correctly.
